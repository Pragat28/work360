...
// ─── User Picker (role-based search; not scoped to a single project here) ────
...
function SubtaskAccordionCard({ subtask, project, onEdit, onRequestDelete, onRequestRate }) {
  ...
  const projectManagers = (project?.assignedManagers || []).map((m) => m.name || m).filter(Boolean);
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              {projectManagers.length > 0 && (
                <span style={styles.managerChip} title="Managers assigned to this project">
...
export default function SubtaskPage() {
  ...
  // Load projects once — this is the single source of truth for managers, employees,
  // and their project assignments. (The dedicated /users?role= endpoint doesn't exist
  // on this backend, so managers/employees are derived from project assignments instead.)
  useEffect(() => {
  ...
  // Deduplicated manager/employee lists, built from every project's assignedManagers /
  // assignedEmployees. Falls back gracefully if a project only has raw ids (no populated name).
  const allManagers = useMemo(() => {
    const map = new Map();
    projects.forEach((p) => {
      (p.assignedManagers || []).forEach((m) => {
        const id = idOf(m);
        if (id && !map.has(id)) map.set(id, typeof m === "object" ? m : { _id: id, name: id });
      });
    });
  ...
  // Reset the employee selection whenever the manager selection changes, since the
  // set of employees that makes sense to pick from can change.
  useEffect(() => {
    setFilters((f) => (f.employeeId === "all" ? f : { ...f, employeeId: "all" }));
  }, [filters.managerId]);
  ...
  // Fetch the full subtask list once. All filtering below happens client-side against
  // project/user relationships, since the backend doesn't reliably support filtering
  // subtasks by managerId/employeeId/projectId directly.
  ...
  // Employees scoped to the selected manager's projects, for the Employee filter dropdown.
  const employeeOptions = useMemo(() => {
    if (filters.managerId === "all") return allEmployees;
    const managerProjectIds = new Set(
      projects.filter((p) => (p.assignedManagers || []).some((m) => idOf(m) === filters.managerId)).map((p) => p._id)
    );
    const scopedIds = new Set();
    projects.forEach((p) => {
      if (managerProjectIds.has(p._id)) {
        (p.assignedEmployees || []).forEach((e) => scopedIds.add(idOf(e)));
      }
    });
    return allEmployees.filter((e) => scopedIds.has(e._id));
  }, [allEmployees, projects, filters.managerId]);

  // All filtering — status, project, manager (via the project's assigned managers),
  // employee (via the subtask's own assignedTo list), and due-date range — computed here.
  const filteredSubtasks = useMemo(() => {
    return subtasks.filter((s) => {
      ...
      if (filters.managerId !== "all") {
        ...
        const managerIds = (project?.assignedManagers || []).map(idOf);
        if (!managerIds.includes(filters.managerId)) return false;
      }
  ...
  const clearFilters = () =>
    setFilters({ status: "all", managerId: "all", employeeId: "all", projectId: "all", startDate: "", endDate: "" });
  ...
  return (
    <main style={styles.main}>
      <header style={{ marginBottom: 4 }}>
        ...
        <p style={styles.pageSubtitle}>Track subtask progress across every manager and project.</p>
      </header>
      ...
      <div style={styles.filterCard}>
        <div style={styles.filterGrid}>
          ...
          <FilterSelect
            ...
            onChange={(v) => updateFilter("managerId", v)}
            options={[{ value: "all", label: "All managers" }, ...allManagers.map((m) => ({ value: m._id, label: m.name }))]}
          />
          <FilterSelect
            label="Employee"
            value={filters.employeeId}
            onChange={(v) => updateFilter("employeeId", v)}
            options={[{ value: "all", label: "All employees" }, ...employeeOptions.map((e) => ({ value: e._id, label: e.name }))]}
          />
          <FilterSelect
            label="Project"
...
const styles = {
  ...
  managerChip: { fontSize: 10, fontWeight: 600, color: "#7c3aed", background: "#ede9fe", borderRadius: 20, padding: "2px 8px" },

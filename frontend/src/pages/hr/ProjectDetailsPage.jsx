  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const pool = (role === "employee" && projectEmployees.length)
...
// ─── Submissions Panel ────────────────────────────────────────────────────────
function SubmissionsPanel({ subtask, currentUser }) {
...
// ─── Remove Member Modal ───────────────────────────────────────────────────────
function RemoveMemberModal({ type, projectId, members, onClose, onRemoved }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  ...
  async function removeUser(userId) {
    ...
    try {
      const field = type === "manager" ? "assignedManagers" : "assignedEmployees";
      const newIds = members.filter(m => m._id !== userId).map(m => m._id);
      const { data } = await API.patch(`/projects/${projectId}`, { [field]: newIds });
      const updated = type === "manager" ? data.project.assignedManagers : data.project.assignedEmployees;
      onRemoved(updated);
    } catch (e) {
  ...
  return (
    <div style={styles.overlay}>
      <div style={{ ...styles.modal, maxWidth: 400 }}>
        <div style={styles.modalHeader}>
          <h2 style={styles.modalTitle}>Remove {type === "manager" ? "Manager" : "Employee"}</h2>
          <button onClick={onClose} style={styles.closeBtn}>✕</button>
        </div>
        <div style={styles.modalBody}>
          {error && <div style={styles.errorBanner}>{error}</div>}
          {members.length === 0 && (
            <div style={{ fontSize: 13, color: "#94a3b8" }}>No {type}s to remove.</div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 280, overflowY: "auto" }}>
            {members.map((u, i) => (
              <div key={u._id} style={styles.userPickRow}>
                ...
                <button onClick={() => removeUser(u._id)} disabled={loading}
                  style={{ padding: "4px 10px", background: "#fef2f2", color: "#dc2626", border: "1px solid #fca5a5", borderRadius: 6, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                  Remove
                </button>
              </div>
            ))}
...
// ─── Subtask Card ─────────────────────────────────────────────────────────────
function SubtaskCard({ subtask, index, total, projectId, projectEmployees, onEdit, onRequestDelete, onRequestRate, onReorder, currentUser }) {
  ...
  useEffect(() => { setLocalSubtask(subtask); setDueVal(fmtDate(subtask.dueDate)); }, [subtask]);
  ...
  const isLate = localSubtask.status === "completed" && !!localSubtask.dueDate && !!localSubtask.completedAt && new Date(localSubtask.completedAt) > new Date(localSubtask.dueDate);
  const flagged = isOverdue || isLate;

  return (
    <div style={{ ...styles.subtaskCard, ...(expanded ? styles.subtaskCardExpanded : {}), ...(flagged ? { background: "#fefce8", border: "1px solid #fde68a" } : {}) }}>
      <div style={styles.subtaskRow} onClick={() => setExpanded(!expanded)}>
        <div style={styles.subtaskLeft}>
          <div style={{ display: "flex", flexDirection: "column", gap: 1, marginRight: 2 }} onClick={e => e.stopPropagation()}>
            <button disabled={index === 0} onClick={() => onReorder(index, -1)} style={{ ...styles.reorderBtn, opacity: index === 0 ? 0.3 : 1 }}>▲</button>
            <button disabled={index === total - 1} onClick={() => onReorder(index, 1)} style={{ ...styles.reorderBtn, opacity: index === total - 1 ? 0.3 : 1 }}>▼</button>
          </div>
            ...
            <div style={styles.subtaskMeta}>
              ...
              {editingDue ? (
                <span onClick={e => e.stopPropagation()} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                  ...
                  <button onClick={saveDueDate} disabled={savingDue} style={{ ...styles.submitBtn, padding: "2px 8px", fontSize: 11 }}>{savingDue ? "…" : "✓"}</button>
                  <button onClick={() => { setDueVal(fmtDate(localSubtask.dueDate)); setEditingDue(false); }} style={{ ...styles.cancelBtn, padding: "2px 8px", fontSize: 11 }}>✕</button>
                </span>
              ) : (
                <span onClick={e => { e.stopPropagation(); setEditingDue(true); }} title="Click to change deadline" style={{ cursor: "pointer", color: flagged ? "#ca8a04" : "#6366f1", fontWeight: flagged ? 600 : 400, borderBottom: `1px dashed ${flagged ? "#ca8a04" : "#6366f1"}` }}>
                  {fmtDate(localSubtask.dueDate)} ✎
                </span>
              )}
              {isOverdue && <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 700, color: "#a16207", background: "#fef9c3", padding: "2px 8px", borderRadius: 8, border: "1px solid #fde68a" }}>⏰ Overdue</span>}
              {isLate && <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 700, color: "#a16207", background: "#fef9c3", padding: "2px 8px", borderRadius: 8, border: "1px solid #fde68a" }}>⏰ Completed late</span>}
            </div>
        ...
        <div style={styles.subtaskRight}>
          ...
          {localSubtask.status === "completed" && (
            <button onClick={e => { e.stopPropagation(); onRequestRate(localSubtask, rating); }} style={{ ...styles.actionBtn, background: "#fef9c3", color: "#ca8a04" }}>
        ...
        <div style={styles.commentsSection}>
          ...
          <div style={{ display: "flex", gap: 2, marginBottom: 14, background: "#f8fafc", borderRadius: 8, padding: 3, alignSelf: "flex-start", width: "fit-content" }}>
            {["comments", "submissions"].map(sec => (
              <button key={sec} onClick={() => setActiveSection(sec)} style={{ padding: "5px 14px", borderRadius: 6, border: "none", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", background: activeSection === sec ? "#6366f1" : "transparent", color: activeSection === sec ? "#fff" : "#64748b" }}>
            ...
            <>
              ...
              {comments.map((c, i) => (
                <div key={c._id || i} style={styles.commentItem}>
                  ...
                  <button onClick={() => deleteComment(c._id)} style={{ ...styles.iconBtn, color: "#ef4444", alignSelf: "flex-start", marginLeft: 6 }} title="Delete comment">🗑</button>
                </div>
              ))}
              <div style={styles.commentInput}>
                <input style={styles.commentBox} placeholder="Write a comment…" value={commentText} onChange={e => setCommentText(e.target.value)} onKeyDown={e => e.key === "Enter" && postComment()} />
                <button onClick={postComment} disabled={postingComment} style={styles.postBtn}>{postingComment ? "…" : "Send"}</button>
              </div>
            </>
          )}

          {activeSection === "submissions" && <SubmissionsPanel subtask={localSubtask} currentUser={currentUser} />}
        </div>
      )}
    </div>
  );
...
// ─── Gantt Timeline (Updated with stacked inline metadata dates) ──────────────
        ...
        <div style={{ minWidth: 700, position: "relative" }}>
          ...
          <div style={{ position: "relative" }}>
            {subtasks.map((s) => {
              ...
              return (
                <div key={s._id} style={{ ...styles.ganttRow, height: "54px" }}> {/* Expanded height for multi-line labels */}
                  ...
                  <div style={styles.ganttBarTrack}>
                    ...
                    >
                      {/* Flex container changed to column layout for elegant multi-line tracking */}
                      <div style={{ ...styles.floatingMetaLabel, flexDirection: "column", alignItems: "flex-start", gap: "2px" }}>
                        ...
                        {/* Task date schedule line added safely underneath */}
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 18, flexWrap: "wrap" }}>
        {FILTERS.map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)}
            style={{ padding: "5px 13px", borderRadius: 20, border: "1px solid", borderColor: filter === f.key ? "#6366f1" : "#e2e8f0", background: filter === f.key ? "#eef2ff" : "#fff", color: filter === f.key ? "#4f46e5" : "#64748b", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ position: "relative" }}>
        {groups.map((group, gi) => (
          <div key={group.key} style={{ marginBottom: gi < groups.length - 1 ? 22 : 0 }}>
            ...
            <div style={{ position: "relative" }}>
              {group.events.length > 1 && <div style={styles.timelineRail} />}
              {group.events.map((ev, i) => {
                const meta = EVENT_META[ev.eventType] || DEFAULT_EVENT_META;
                return (
                  <div key={ev._id || i} style={styles.timelineRow}>
                    <div style={{ ...styles.timelineDot, background: meta.bg, color: meta.color, border: `2px solid ${meta.color}33` }}>{meta.icon}</div>
                    <div style={styles.timelineContent}>
                      <div style={styles.timelineDescription}>{ev.description}</div>
                      <div style={styles.timelineTime}>{new Date(ev.createdAt).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</div>
                    </div>
                  </div>
                );
              })}
...
function EditProjectInline({ project, onClose, onSaved }) {
  ...
  async function submit() { setLoading(true); try { await onSaved(form); } finally { setLoading(false); } }
...
// ─── HR Project Detail Page ───────────────────────────────────────────────────
export default function HRProjectDetailPage() {
  const { id: projectId } = useParams();
  const navigate = useNavigate();
  const onBack = () => navigate("/hr/projects");
  ...
  const [currentUser] = useState(() => {
    try {
      ...
      return { ...u, role: "hr_admin" };
    } catch { return { role: "hr_admin" }; }
  });
  ...
  function handleManagerRemoved(managers) { setProject(prev => ({ ...prev, assignedManagers: managers })); setModal(null); }
  function handleEmployeeAdded(employees) { setProject(prev => ({ ...prev, assignedEmployees: employees })); setModal(null); }
  function handleEmployeeRemoved(employees) { setProject(prev => ({ ...prev, assignedEmployees: employees })); setModal(null); }
  ...
  return (
    <main style={styles.main}>
      ...
      <div style={styles.projectHeader}>
        <div style={styles.projectHeaderLeft}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20, background: "#ede9fe", color: "#6d28d9" }}>HR View</span>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {subtasks.length === 0 && (
            <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 12, padding: 40, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 14, color: "#64748b", marginTop: 8 }}>No subtasks yet.</div>
            </div>
          )}
          {subtasks.map((subtask, index) => (
            <SubtaskCard key={subtask._id} subtask={subtask} index={index} total={subtasks.length}
              projectId={projectId} projectEmployees={employees} currentUser={currentUser}
          ...
          <div style={styles.card}>
            <div style={styles.cardHeader}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setModal({ type: "add-member", memberType: "manager" })} style={styles.addMemberBtn}>+ Add Manager</button>
                {managers.length > 0 && (
                  <button onClick={() => setModal({ type: "remove-member", memberType: "manager" })} style={{ ...styles.addMemberBtn, background: "#fef2f2", color: "#dc2626", borderColor: "#fca5a5" }}>− Remove Manager</button>
                )}
              </div>
            </div>
              ...
              {managers.map((m, i) => (
                <div key={m._id} style={styles.memberCard}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#6d28d9", background: "#ede9fe", borderRadius: 10, padding: "1px 6px", display: "inline-block", marginTop: 3 }}>Manager</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={styles.card}>
            <div style={styles.cardHeader}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                {employees.length > 0 && (
                  <button onClick={() => setModal({ type: "remove-member", memberType: "employee" })} style={{ ...styles.addMemberBtn, background: "#fef2f2", color: "#dc2626", borderColor: "#fca5a5" }}>− Remove Employee</button>
                )}
              </div>
            </div>
            <div style={styles.memberGrid}>
      ...
      {/* Modals Mapping */}
      {modal === "add-subtask" && <AddSubtaskModal projectId={projectId} projectEmployees={employees} onClose={() => setModal(null)} onCreated={handleSubtaskCreated} />}
      {modal?.type === "edit-subtask" && <EditSubtaskModal subtask={modal.subtask} projectId={projectId} projectEmployees={employees} onClose={() => setModal(null)} onSaved={handleSubtaskSaved} />}
      {modal?.type === "rate" && <RatingModal subtask={modal.subtask} existingRating={modal.existingRating} onClose={() => setModal(null)} onSubmitted={handleRated} />}
      {modal?.type === "add-member" && <AddMemberModal type={modal.memberType} projectId={projectId} currentIds={(modal.memberType === "manager" ? managers : employees).map(m => m._id)} onClose={() => setModal(null)} onAdded={modal.memberType === "manager" ? handleManagerAdded : handleEmployeeAdded} />}
      {modal?.type === "remove-member" && <RemoveMemberModal type={modal.memberType} projectId={projectId} members={modal.memberType === "manager" ? managers : employees} onClose={() => setModal(null)} onRemoved={modal.memberType === "manager" ? handleManagerRemoved : handleEmployeeRemoved} />}
      {modal === "edit-project" && <EditProjectInline project={project} onClose={() => setModal(null)} onSaved={handleProjectSaved} />}
